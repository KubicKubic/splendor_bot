...
- 所有座位的预定牌身份公开，牌堆仅公开各层剩余张数；模型不读取暗牌身份、未来牌序，
...
奖励为终局零和胜负（`m<n` 时胜者 `+1/m`、败者 `-1/(n-m)`；全员并列 `m=n`
时所有人均为 `0`）加势函数差分塑形，势函数来自公开分数和已购卡数量。
塑形在终局归零，避免永久累积买卡奖励。模型为前馈网络，尚未加入历史记忆或搜索。

`max_turns` 只作为异常长局诊断阈值并记录 `long_games`，不会截断或把长局改成和局；
