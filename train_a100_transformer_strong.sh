...
exec bash train_a100.sh \
  ...
  --architecture pooled_transformer --width 448 \
  --observation-version 3 \
  --transformer-layers 4 --transformer-heads 4 --transformer-ff-dim 448 \
  --token-embed-width 48 \
  --policy-head-width 352 --policy-head-layers 2 \
  --value-head-width 288 --value-head-layers 2 \
